...
- **Pre-trade risk in C++.** Kill-switch, max order volume, max notional, price-deviation (fat-finger) guard, max total open-position cost, and a token-bucket rate limiter are enforced on the order path in native code — a JS GC pause can't defeat them.
